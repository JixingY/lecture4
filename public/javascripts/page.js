 ...
 * - Send an AJAX GET request to the server
 * - HTML of posts sent in response
 * - HTML injected directly in page
 */
function loadPostsHtml() {
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            // Replace the contents of the post-list section with HTML sent in response.
            // Note, this HTML is not a full page, just the snippet we want to inject.
            document.getElementById('post-list').innerHTML = this.responseText;
        }
    };

    // Open connection to server
    xmlhttp.open("GET", "/posthtml", true);
